...
{
    if (elements.empty()) {
        THROW_ERROR_EXCEPTION("Empty elements list");
    }
    ...
    for (   Elements::const_iterator it = elements.begin();
        ...
        if (it->size() != MERKLE_TREE_ELEMENT_SIZE_B) {
            ...
            THROW_ERROR_EXCEPTION(oss.str().c_str());
        }
...
MerkleTree::Elements MerkleTree::getProof(const Buffer& element) const
{
    ...
    if (!found) {
        THROW_ERROR_EXCEPTION("Element not found");
    }
...
{
    if (index == 0) {
        THROW_ERROR_EXCEPTION("Index is zero");
    }
    index--;
    if ((index >= elements_.size()) || (elements_[index] != element)) {
        THROW_ERROR_EXCEPTION("Index does not point to element");
    }
    return getProof(index);
}
